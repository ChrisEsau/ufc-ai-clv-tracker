...
def _probabilities(
    priors: BrainIntentPriors, capabilities: BrainCapabilities | None = None
):
    capabilities = capabilities or BrainCapabilities(
        0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.3, 0.4, 0.3
    )
...
def test_neutral_standing_td_to_strike_odds_equal_fsr_rate_ratio() -> None:
    capabilities = BrainCapabilities(0.5, 0.5, 0.5, 0.5, 0.95, 0.5, 0.3, 0.4, 0.3)
    priors = BrainIntentPriors(
        standing_attempt_rate_15m=90.0, takedown_attempt_rate_15m=4.5
    )
    probs = _probabilities(priors, capabilities)
    strike = probs[ActionFamily.STAND_ATTACK] + probs[ActionFamily.STAND_COUNTER]
    assert probs[ActionFamily.TAKEDOWN_ENTRY] / strike == pytest.approx(
        4.5 / 90.0, rel=1e-12
    )


def test_neutral_clinch_entry_odds_equal_population_prior() -> None:
    priors = BrainIntentPriors(90.0, 4.5, clinch_entry_to_standing_ratio=0.04)
    probs = _probabilities(priors)
    strike = probs[ActionFamily.STAND_ATTACK] + probs[ActionFamily.STAND_COUNTER]
    assert probs[ActionFamily.CLINCH_ENTRY] / strike == pytest.approx(0.04, rel=1e-12)


def test_clinch_capability_does_not_change_neutral_entry_odds_when_prior_fixed() -> (
    None
):
    low = BrainCapabilities(0.5, 0.5, 0.5, 0.0, 0.5, 0.5, 0.3, 0.4, 0.3)
    high = BrainCapabilities(0.5, 0.5, 0.5, 1.0, 0.5, 0.5, 0.3, 0.4, 0.3)
    priors = BrainIntentPriors(80.0, 4.0, clinch_entry_to_standing_ratio=0.04)
    low_probs = _probabilities(priors, low)
    high_probs = _probabilities(priors, high)
    low_strike = (
        low_probs[ActionFamily.STAND_ATTACK] + low_probs[ActionFamily.STAND_COUNTER]
    )
    high_strike = (
        high_probs[ActionFamily.STAND_ATTACK] + high_probs[ActionFamily.STAND_COUNTER]
    )
    assert low_probs[ActionFamily.CLINCH_ENTRY] / low_strike == pytest.approx(
        0.04, rel=1e-12
    )
    assert high_probs[ActionFamily.CLINCH_ENTRY] / high_strike == pytest.approx(
        0.04, rel=1e-12
    )


def test_completion_like_capability_does_not_change_neutral_td_odds_when_priors_fixed() -> (
    None
):
    low = BrainCapabilities(0.5, 0.5, 0.5, 0.5, 0.0, 0.5, 0.3, 0.4, 0.3)
    high = BrainCapabilities(0.5, 0.5, 0.5, 0.5, 1.0, 0.5, 0.3, 0.4, 0.3)
...
def test_recent_success_and_failure_still_modulate_td_choice() -> None:
    capabilities = BrainCapabilities(0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.3, 0.4, 0.3)
    priors = BrainIntentPriors(80.0, 4.0)
    ...
    assert p(BrainDecisionContext(td_success_recent=0.8)) > neutral
    assert p(BrainDecisionContext(td_failure_recent=0.8)) < neutral
...
def test_intent_priors_validate_rate_semantics() -> None:
    ...
    with pytest.raises(ValueError):
        BrainIntentPriors(10.0, 1.0, clinch_entry_to_standing_ratio=-0.1)
...
def test_ground_structural_multipliers_raise_strikes_and_reduce_submissions():
    ...
    corrected = BrainIntentPriors(
        80.0, 4.0, ground_strike_odds_multiplier=3.0, submission_odds_multiplier=0.3
    )
    p0 = {
        r.action_family: r.probability
        for r in action_probabilities_with_intent_priors(
            state,
            Side.RED,
            BrainCapabilities(0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.3, 0.4, 0.3),
