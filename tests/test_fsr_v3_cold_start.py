...
def test_external_strength_reduces_prior_sd_and_moves_mean():
    base = combine_positive_rate_prior(
...
def test_strength_calibration_keeps_zero_when_external_equals_population():
    frame = pd.DataFrame(
        {
    ...
    # Identical means can change dispersion slightly, so the calibration must at
    # least return a valid tested strength rather than assuming external value.
