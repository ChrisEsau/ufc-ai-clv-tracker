...
# Official UFC event-page prices captured for UFC Shanghai, Aug. 29, 2026.
# First listed fighter is treated as red for feature orientation only.
FIGHTS = [
...
def add_live_engineered(livefv: pd.DataFrame) -> pd.DataFrame:
    def _num(c):
        return pd.to_numeric(livefv[c], errors="coerce") if c in livefv.columns else pd.Series(np.nan, index=livefv.index)
    def _rate(c):
        s = _num(c)
        return pd.Series(np.where(s > 1, s / 100.0, s), index=livefv.index)
...
def fit_frozen_v5():
    ...
    for repo_path, out_path in [
        ("data/market/historical_market_outcomes.parquet", "/tmp/v5_market.parquet"),
        ("data/features/moneyline_feature_view.parquet", "/tmp/v5_fv.parquet"),
    ]:
        with open(out_path, "wb") as f:
            subprocess.run(["git", "show", f"{SNAP}:{repo_path}"], stdout=f, check=True)
    ...
    ml = ml[ml.fight_id.isin(good[good == 2].index)].copy()
    ...
    fv = pd.read_parquet("/tmp/v5_fv.parquet").copy()

    feature_cols = SELECTED + ["market_overround"]
    df = red.merge(fv[["fight_id"] + [c for c in SELECTED if c in fv.columns]], on="fight_id", how="inner").sort_values(["date", "fight_id"]).copy()
    missing_train = [c for c in SELECTED if c not in df.columns]
    if missing_train:
        raise RuntimeError(f"Frozen V5 training snapshot missing selected features: {missing_train}")
...
def build_prefight_view():
    hist = pd.read_parquet("data/features/fighters_historical.parquet").copy()
    if "fighter_name" not in hist.columns or "fighter_id" not in hist.columns or "date" not in hist.columns:
        raise RuntimeError("fighters_historical lacks fighter_name/fighter_id/date")
    hist["date"] = pd.to_datetime(hist["date"], errors="coerce")
    hist["fighter_id"] = hist.fighter_id.astype(str)
    hist["_norm_name"] = hist.fighter_name.map(norm)

    prep_rows, state_rows, meta = [], [], {}
    for i, (a, b, ao_a, ao_b, rounds) in enumerate(FIGHTS, 1):
        fid = f"shanghai_v5_20260829_{i:02d}"
        ids, found, state_dates = [], [], []
        for nm in (a, b):
            ...
            if len(hit):
                ...
                ok = True
                sd = pd.Timestamp(rec["date"]).strftime("%Y-%m-%d")
            else:
                rec = {c: np.nan for c in hist.columns if c != "_norm_name"}
            ...
            found.append(ok)
            state_dates.append(sd)
        prep_rows.append({"fight_id":fid,"r_id":ids[0],"b_id":ids[1],"r_name":a,"b_name":b,"date":FIGHT_DATE,"title_fight":False,"total_rounds":rounds})
        meta[fid] = {"r":(a, ao_a, found[0], state_dates[0]), "b":(b, ao_b, found[1], state_dates[1])}

    livefv = build_moneyline_feature_view(prepared_fights_df=pd.DataFrame(prep_rows), fighter_state_history_df=pd.DataFrame(state_rows))
    livefv = add_live_engineered(livefv)
    missing_live = [c for c in SELECTED if c not in livefv.columns]
    if missing_live:
        raise RuntimeError(f"Prefight feature view missing frozen V5 features: {missing_live}")
    return livefv, meta


def main():
    model, valid, med = fit_frozen_v5()
    livefv, meta = build_prefight_view()
    rows = []
    for _, r in livefv.iterrows():
        a, ao_a, found_a, date_a = meta[r.fight_id]["r"]
        b, ao_b, found_b, date_b = meta[r.fight_id]["b"]
        ipa, ipb = imp(ao_a), imp(ao_b)
        over = ipa + ipb
        fair_a, fair_b = ipa / over, ipb / over
        vals = {c: r[c] for c in SELECTED}
        vals["market_overround"] = over
        xlive = pd.DataFrame([{c: vals.get(c, np.nan) for c in valid}], columns=valid).replace([np.inf, -np.inf], np.nan).fillna(med).fillna(0.0)
        mlive = logit([fair_a])
        dlive = xgb.DMatrix(xlive, base_margin=mlive, feature_names=valid)
        full_margin = model.predict(dlive, output_margin=True)
        correction = full_margin - mlive
        p_a = float(sigmoid(mlive + correction)[0])
        p_b = 1.0 - p_a
        cold = not (found_a and found_b)
        common = {"market_source":"UFC event page 2026-08-29","fight_cold_start":cold,"selected_feature_count":len(SELECTED)+1,"market_overround":over}
        rows += [
            {"fight_id":r.fight_id,"fighter":a,"opponent":b,"side":"red","american_odds":ao_a,"fighter_state_found":found_a,"prefight_state_date":date_a,"fair_market_p":fair_a,"v5_model_p":p_a,"edge":p_a-fair_a,**common},
            {"fight_id":r.fight_id,"fighter":b,"opponent":a,"side":"blue","american_odds":ao_b,"fighter_state_found":found_b,"prefight_state_date":date_b,"fair_market_p":fair_b,"v5_model_p":p_b,"edge":p_b-fair_b,**common},
        ]
    out = pd.DataFrame(rows).sort_values(["fight_id","side"])
    out.to_csv(OUTPUT, index=False)
    print(out.to_string(index=False))
    print("missing_states=", out.loc[~out.fighter_state_found, "fighter"].tolist())
    print("eligible_fights=", int((~out.groupby("fight_id").fight_cold_start.first()).sum()), "/", out.fight_id.nunique())
    print("output=", OUTPUT)
