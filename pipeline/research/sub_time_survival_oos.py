"""Research-only censored submission time-survival study.

Exact statistical mirror of pipeline.research.ko_time_survival_oos, with only the
finish event changed from KO/TKO to submission.

Each fighter-side is a survival process:
  * submission winner: event at observed finish time
  * opponent in that submission fight: censored at the same time
...
OUT = Path("data/research/sub_time_survival_oos")
...
def _submission(s: pd.Series) -> pd.Series:
    t = s.fillna("").astype(str).str.upper()
    return t.str.contains(r"SUBMISSION|\\bSUB\\b", regex=True)


def load_fighter_fights() -> pd.DataFrame:
    # Use round stats for canonical fighter/opponent IDs; no grappling features are used.
    r = pd.read_parquet(
        ROUND_STATS_PATH,
        columns=["event_date", "fight_id", "fighter_id", "fighter_name", "opponent_id"],
    )
    ...
    cols = [
        "fight_id", "date", "method", "winner_id", "r_id", "b_id", "division", "match_time_sec"
    ]
    ...
    ff["sub_event"] = (ff.won & _submission(ff.method)).astype(int)
    ff["sub_loss"] = ((~ff.won) & _submission(ff.method)).astype(int)
...
def add_prefight(ff: pd.DataFrame) -> pd.DataFrame:
    state = defaultdict(lambda: {"seconds": 0.0, "sub_win": 0.0, "sub_loss": 0.0})
    rows = []
    for _, batch in ff.groupby("event_date", sort=True):
        for row in batch.itertuples(index=False):
            ...
            d["prior_sub_win"] = s["sub_win"]
            d["prior_sub_loss"] = s["sub_loss"]
            rows.append(d)
        for row in batch.itertuples(index=False):
            s = state[str(row.fighter_id)]
            s["seconds"] += float(row.fight_seconds)
            s["sub_win"] += float(row.sub_event)
            s["sub_loss"] += float(row.sub_loss)
    x = pd.DataFrame(rows)
    opp = x[[
        "event_date", "fight_id", "fighter_id", "prior_seconds", "prior_sub_loss"
    ]].rename(columns={
        "fighter_id": "opponent_id",
        "prior_seconds": "opp_prior_seconds",
        "prior_sub_loss": "opp_prior_sub_loss",
    })
    return x.merge(
        opp,
...
def interval_exposure(t: np.ndarray) -> np.ndarray:
    ...
    return np.stack([
        np.clip(t - CUTS[i], 0.0, CUTS[i + 1] - CUTS[i])
...
def interval_event_index(t: np.ndarray) -> np.ndarray:
    return np.clip(
        np.searchsorted(CUTS[1:], np.asarray(t, float), side="left"),
        0,
        len(CUTS) - 2,
    )
...
def train_baselines(train: pd.DataFrame) -> tuple[float, np.ndarray]:
    ...
    ev = float(train.sub_event.sum())
    p0 = ev / sec
    exp = interval_exposure(train.fight_seconds.to_numpy(float)).sum(axis=0)
    events = np.zeros(len(CUTS) - 1, float)
    idx = interval_event_index(
        train.loc[train.sub_event.eq(1), "fight_seconds"].to_numpy(float)
    )
    for j in idx:
        events[int(j)] += 1.0
    prior_e = 2.0
...
def fighter_rr(test: pd.DataFrame, p0: float, prior_events: float) -> np.ndarray:
    ...
    att = (
        test.prior_sub_win.to_numpy(float) + prior_events
    ) / (
        test.prior_seconds.to_numpy(float) + prior_sec
    )
    deff = (
        test.opp_prior_sub_loss.to_numpy(float) + prior_events
    ) / (
        test.opp_prior_seconds.to_numpy(float) + prior_sec
    )
...
def score_year(test: pd.DataFrame, train: pd.DataFrame, prior_events: float) -> list[dict]:
    ...
    for name, haz in [
        ...
        rows.append({
            "model": name,
            "survival_nll": nll,
            "event_auc_prefight_hazard": float(auc),
            "population_hazard_per_second": p0,
...
def run() -> None:
    ...
    pooled = []
    for period, years in [
        ("selection_2020_2024", SELECTION_YEARS),
        ("confirmation_2025_2026", CONFIRMATION_YEARS),
    ]:
        ...
        for (model, pe), z in g.groupby(["model", "prior_events"]):
            ...
            pooled.append({
                "period": period,
                "model": model,
                "prior_events": float(pe),
                "n": int(w.sum()),
                "events": int(z.events.sum()),
                "survival_nll": float(np.average(z.survival_nll, weights=w)),
                "event_auc_prefight_hazard": float(
                    np.average(z.event_auc_prefight_hazard, weights=w)
                ),
            })
    pooled = pd.DataFrame(pooled)
    sel = pooled[pooled.period.eq("selection_2020_2024")].sort_values(
        ["survival_nll", "event_auc_prefight_hazard"], ascending=[True, False]
    )
    best = sel.groupby("model", as_index=False).first().sort_values("survival_nll")
    chosen = best[["model", "prior_events"]]
    confirm = pooled[pooled.period.eq("confirmation_2025_2026")].merge(
        chosen, on=["model", "prior_events"], how="inner"
    ).sort_values("survival_nll")

    ev = ff[ff.sub_event.eq(1)].copy()
    event_bins = pd.cut(ev.fight_seconds, bins=CUTS, right=True, include_lowest=True)
    event_dist = ev.groupby(event_bins, observed=False).size().reset_index(name="sub_events")
    event_dist["share"] = event_dist.sub_events / max(event_dist.sub_events.sum(), 1)
    ...
    event_dist.to_csv(OUT / "sub_event_time_distribution.csv", index=False)
    report = {
        "study": "censored submission time survival",
    ...
    print("SUB TIME SURVIVAL — SELECTION BEST")
    print(best.to_string(index=False))
    print("\nSUB TIME SURVIVAL — UNTOUCHED 2025-2026 CONFIRMATION")
    print(confirm.to_string(index=False))
    print("\nSUB EVENT TIME DISTRIBUTION")
