...
BASE_RATING = 1000.0
K_FACTOR = 170.0
OUT = Path("data/diagnostics/elo_fsr_brain_10_fight_shadow")
...
def main() -> None:
    ...
    for n, fight_id in enumerate(FIGHT_IDS, start=1):
        ...
        print(f"[{n}/{len(FIGHT_IDS)}] {fighter_a} vs {fighter_b} | Elo {elo_a:.1f}-{elo_b:.1f}", flush=True)
    ...
    pd.DataFrame(all_audit).to_csv(OUT / "elo_active_input_adjustments.csv", index=False)
    (OUT / "summary.json").write_text(json.dumps({"fights": summaries}, indent=2, default=str) + "\n")
    print(json.dumps({"fight_count": len(summaries), "output": str(OUT)}, indent=2))
