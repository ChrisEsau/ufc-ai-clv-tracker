...
def _fit_eval(train, test, features, target):
    ...
    return {
        ...
        "standardized_coefficients": {
            name: float(v) for name, v in zip(features, lr.coef_[0])
        },
...
def main():
    ...
    if missing:
        raise RuntimeError(
            f"missing submission traits in canonical prefight snapshots: {missing}"
        )
    ...
    opp = snapshots[
        ["event_date", "fight_id", "fighter_id", "submission_defense"]
    ].copy()
    opp = opp.rename(
        columns={
            "fighter_id": "opponent_id",
            "submission_defense": "opponent_submission_defense",
        }
    )
    frame = frame.merge(
        opp,
    ...
    frame["offense_defense_edge"] = (
        frame["submission_offense"] - frame["opponent_submission_defense"]
    )
    clipped = np.clip(frame["submission_conversion_baseline"], 1e-8, 1 - 1e-8)
    frame["baseline_logit"] = np.log(clipped / (1 - clipped))
    ...
    a_train = train[train.ground_exposure_seconds > 0].dropna(
        subset=["submission_tendency"]
    )
    a_test = test[test.ground_exposure_seconds > 0].dropna(
        subset=["submission_tendency"]
    )
    attempt_base = _fit_eval(
        a_train, a_test, ["log_ground_seconds"], "any_submission_attempt"
    )
    attempt_trait = _fit_eval(
        a_train,
        a_test,
        ["log_ground_seconds", "submission_tendency"],
        "any_submission_attempt",
    )

    rate_rows = frame[
        (frame.ground_exposure_seconds > 0) & frame.submission_tendency.notna()
    ]
    rho, rho_p = spearmanr(
        rate_rows["submission_tendency"],
        rate_rows["attempts_per_ground_min"],
        nan_policy="omit",
    )
    ...
    c_train = train[train.submission_attempts > 0].dropna(
        subset=["baseline_logit", "offense_defense_edge"]
    )
    c_test = test[test.submission_attempts > 0].dropna(
        subset=["baseline_logit", "offense_defense_edge"]
    )
    conv_attempt_only = _fit_eval(
        c_train, c_test, ["log_attempts"], "submission_finish"
    )
    conv_baseline = _fit_eval(
        c_train,
        c_test,
        ["log_attempts", "baseline_logit"],
        "submission_finish",
    )
    conv_matchup = _fit_eval(
        c_train,
        c_test,
        ["log_attempts", "baseline_logit", "offense_defense_edge"],
        "submission_finish",
    )
    ...
    result = {
        ...
        "attempt_propensity": {
            ...
            "delta_auc": (
                None
                if attempt_base["auc"] is None or attempt_trait["auc"] is None
                else float(attempt_trait["auc"] - attempt_base["auc"])
            ),
            "delta_log_loss": float(
                attempt_trait["log_loss"] - attempt_base["log_loss"]
            ),
            "spearman_tendency_vs_attempts_per_ground_min": float(rho),
            "spearman_p": float(rho_p),
            "tendency_quintiles": _quintiles(
                rate_rows, "submission_tendency", "any_submission_attempt"
            ),
        },
        "conversion": {
            "limitation": (
                "UFCStats has submission-attempt counts but not attempt-level "
                "timestamps/outcomes; models predict fighter-fight submission "
                "finish among attempt-positive rows and control for attempt count."
            ),
            ...
            "delta_auc_matchup_vs_baseline": (
                None
                if conv_baseline["auc"] is None or conv_matchup["auc"] is None
                else float(conv_matchup["auc"] - conv_baseline["auc"])
            ),
            "delta_log_loss_matchup_vs_baseline": float(
                conv_matchup["log_loss"] - conv_baseline["log_loss"]
            ),
            "edge_quintiles": _quintiles(
                c_test, "offense_defense_edge", "submission_finish"
            ),
        },
    }
