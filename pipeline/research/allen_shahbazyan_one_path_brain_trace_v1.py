...
from pipeline.simulation.event_clock_mc_v2.diagnostics.leavitt_brito_intent_rate_shadow import IntentRateBrain, _standing_rates
...
class TraceBrain(IntentRateBrain):
    ...
    def action_chooser(self,state,actor,capabilities,context,rng,config):
        if state.phase is Phase.STANDING:
            rates,pres=_standing_rates(state,actor,capabilities,context,self.priors[actor],config)
            ...
            dist=action_probabilities_with_intent_priors(state,actor,capabilities,context,self.priors[actor],config)
            rows=[{'action':r.action_family.value,'utility':float(r.utility),'probability':float(r.probability)} for r in dist]
            probs=[r.probability for r in dist]; i=int(rng.choice(len(dist),p=probs)); selected=dist[i].action_family; extra={}
