...
def _parse_args(argv=None):
    ...
    parser.add_argument(
        "--fight-id",
        required=True,
        help="target fight_id from repository master/prefight data",
    )
    parser.add_argument(
        "--paths",
        type=int,
        ...
        help=(
            f"number of matched-seed paths (default: {LOCKED_PATHS}); "
...
def _set_fight_id(fight_id: str) -> None:
    """Set target consistently through every shared seam used by locked stack."""
...
def _round_budget_resolver_class(original_cls):
    """Apply one sampled round-total control budget per controller per round."""
    ...
    class RoundBudgetEscapeResolver(original_cls):
        ...
        def _ensure_budget(self, state) -> float:
            ...
            if key not in self.round_budgets:
                ...
                budget = float(
                    np.clip(
                        expected * ratio,
                        timing.target.MIN_DURATION,
                        timing.target.MAX_DURATION,
                    )
                )
        ...
        def _consume_spell(self, state, elapsed: float) -> None:
            ...
            self.round_consumed[key] = min(
                budget, prior + max(float(elapsed), 0.0)
            )
...
def _primary_mechanic_probability(event, state, inputs, placeholders) -> dict:
    ...
    if family in {ActionFamily.STAND_ATTACK, ActionFamily.STAND_COUNTER}:
        return {
            "mechanic": "standing_strike_landing",
            "probability": float(fighter.standing_strike_landing_probability),
        }
    if family is ActionFamily.CLINCH_ENTRY:
        return {
            "mechanic": "clinch_entry_success",
            "probability": float(placeholders.clinch_entry_success_probability),
        }
    if family in {ActionFamily.TAKEDOWN_ENTRY, ActionFamily.CLINCH_TAKEDOWN}:
        return {
            "mechanic": "takedown_completion",
            "probability": float(fighter.takedown_completion_probability),
        }
    if family is ActionFamily.CLINCH_STRIKE:
        return {
            "mechanic": "clinch_strike_landing",
            "probability": float(placeholders.clinch_strike_landing_probability),
        }
    if family is ActionFamily.BREAK_CLINCH:
        return {
            "mechanic": "break_clinch_success",
            "probability": float(placeholders.break_clinch_success_probability),
        }
    if family in {ActionFamily.GROUND_STRIKE, ActionFamily.BOTTOM_STRIKE}:
        return {
            "mechanic": "ground_strike_landing",
            "probability": float(fighter.ground_strike_landing_probability),
        }
    if family is ActionFamily.REVERSAL:
        return {
            "mechanic": "ground_reversal",
            "probability": float(fighter.ground_reversal_probability),
        }
    if family is ActionFamily.ESCAPE_STAND:
        return {
            "mechanic": "round_control_budget_escape_threshold",
            "probability": None,
        }
            ...
            pre_mechanics.append(
                {
                    ...
                    **_primary_mechanic_probability(
                        event, state, mechanics_inputs, placeholders
                    ),
                }
            )
            ...
            rows.append(
                {
                    ...
                    "seconds_since_prior_event": (
                        float(event.timestamp_seconds) - previous_timestamp
                    ),
...
def _write_event_report(captured: dict, fight_id: str, outdir: Path) -> None:
    if not captured:
    ...
    for row in captured["events"]:
        flat = dict(row)
        flat["brain_options"] = json.dumps(
            flat["brain_options"], separators=(",", ":")
        )
        flat["escape_model"] = (
...
def main(*, fight_id: str, paths: int = LOCKED_PATHS) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="locked-brain-canonical-") as td:
        ...
        try:
            ...
            canonical["event_date"] = pd.to_datetime(
            ...
            if len(target) != 2:
                raise RuntimeError(
                    f"expected 2 PURE EWM target rows for {fight_id}, found {len(target)}"
                )
            ...
            manifest = {
                ...
                "event_report_files": (
                    ["event_report.json", "event_report.csv"] if paths == 1 else []
                ),
                ...
                "control_duration_semantics": (
                    "one sampled round-total control budget per controller per "
                    "round; re-entries consume remaining budget"
                ),
                "ko": (
                    "piecewise time-based competing clock from "
                    "allen_shahbazyan_time_ko_clock_2000"
                ),
                "kd": (
                    "OOS-selected static prefight KD hazard; no within-fight "
                    "KD escalation"
                ),
                "submission": (
                    "current locked ground-opportunity/fighter-level submission "
                    "research stack"
                ),
    ...
    (outdir / "restore_verification.json").write_text(
        json.dumps(restore, indent=2) + "\n", encoding="utf-8"
    )
