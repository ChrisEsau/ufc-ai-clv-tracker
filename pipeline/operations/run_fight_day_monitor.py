...
def build_steps(args: argparse.Namespace) -> list[Step]:
    ...
    if not bool(args.official_closing_snapshot):
        closing_command.append("--no-official-closing-snapshot")

    return [
        Step(
            step_id="refresh_live_markets",
            name="Refresh Live Markets",
            substeps=[
                Substep(
        ...
        Step(
            ...
            name="Recalculate Betting Board",
            substeps=[
                Substep(
        ...
        Step(
            ...
            name="Capture Model-Market Snapshot",
            substeps=[
                Substep(
        ...
        Step(
            ...
            substeps=[
                Substep(
                    substep_id="capture_closing_line_snapshot",
                    name="Capture Closing-Line Snapshot",
                    command=closing_command,
                    expected_outputs=[
                        "data/snapshots/closing_line_snapshots.parquet",
                        "data/audits/closing_line_snapshot_audit.parquet",
                    ],
                )
            ],
        ),
...
def run_fight_day_monitor(args: argparse.Namespace) -> None:
    steps = build_steps(args)
    start_runbook(
        runbook_id=RUNBOOK_ID,
        mode=args.mode,
        step_total=len(steps),
        message=f"Fight Day Monitor started in {args.mode} mode",
    )
    ...
    try:
        for step_index, step in enumerate(steps, start=1):
            ...
            for substep_index, substep in enumerate(step.substeps, start=1):
                ...
                complete_substep(f"Completed {substep.name}", runbook_id=RUNBOOK_ID)
            complete_step(f"Completed {step.name}", runbook_id=RUNBOOK_ID)
        complete_runbook("Fight Day Monitor completed", runbook_id=RUNBOOK_ID)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=True,
        help="Whether closing-line rows should be flagged as official. Defaults to true.",
    )
