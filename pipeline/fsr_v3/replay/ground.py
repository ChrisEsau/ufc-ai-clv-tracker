"""Validated FSR V3 ground-striking replay.

This module implements only structures that survived the chronological
validation study:
...
Ground effectiveness
    landed|attempted ~ Beta-Binomial(p, rho=.08)
    logit(p) = beta_population + O_attacker
    O_attacker ~ Normal(0, .25^2)
    There is intentionally no defender ground-effectiveness trait.
    Posterior mean only; c=0.
...
def _bounded_logit_probability(successes: float, trials: float) -> float:
    # Jeffreys-style half-count initialization uses only prior observations.
    p = (float(successes) + 0.5) / (float(trials) + 1.0)
    p = float(np.clip(p, 1e-6, 1.0 - 1e-6))
    return log(p / (1.0 - p))


# =============================================================================
# Ground tendency population fit
# =============================================================================
...
# =============================================================================
# Ground suppression population fit
# =============================================================================
...
def replay_ground_effectiveness(
    ...
    states: dict[str, np.ndarray] = {}
    population_landed = 0.0
    population_attempted = 0.0
    rows: list[dict] = []

    for event_date, batch in fights.groupby("event_date", sort=True):
        beta = _bounded_logit_probability(population_landed, population_attempted)
        baseline_probability = float(expit(beta))
        pending: list[tuple[str, float, float, np.ndarray | None]] = []
        ...
        for fighter, landed, attempted, observation_ll in pending:
            ...
            if fighter in states:
                ...
                states[fighter] = observation_ll - np.max(observation_ll)
            population_landed += landed
            population_attempted += attempted
...
# =============================================================================
# Full validated ground family
# =============================================================================


def replay_ground_family(
    paired_rounds: pd.DataFrame | None = None,
    config: FSRV3Config | None = None,
) -> GroundReplayResult:
    config = config or FSRV3Config()
    fights = build_ground_fighter_fights(paired_rounds)
    ...
    return GroundReplayResult(tendency, suppression, effectiveness)
