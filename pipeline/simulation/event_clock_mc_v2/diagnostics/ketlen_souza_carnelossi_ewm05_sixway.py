"""Ketlen Souza vs Ariane Carnelossi pure-EWM 0.50 shadow with explicit six-way methods."""
...
FIGHTER_A = "Ketlen Souza"
FIGHTER_B = "Ariane Carnelossi"
...
def resolve_fight_id() -> str:
    ...
    if len(rows) != 1:
        raise RuntimeError(f"Expected exactly one {FIGHTER_A} vs {FIGHTER_B} row, found {len(rows)}")
...
def main() -> None:
    ...
    try:
        ...
        payload = {
            "diagnostic": "Ketlen Souza-Carnelossi pure EWM 0.50 six-way shadow",
            "fight_id": fight_id,
            "paths": PATHS,
            "ewm_decay": EWM_DECAY,
            "canonical_blend": 0.0,
            ...
            "fsr_effective_rates": {
                names[Side.RED]: {"standing_rate_15m": priors[Side.RED].standing_attempt_rate_15m, "takedown_rate_15m": priors[Side.RED].takedown_attempt_rate_15m},
                names[Side.BLUE]: {"standing_rate_15m": priors[Side.BLUE].standing_attempt_rate_15m, "takedown_rate_15m": priors[Side.BLUE].takedown_attempt_rate_15m},
            },
            "fighter_methods": fighter_methods,
        }
        print("KETLEN_SOUZA_CARNELOSSI_EWM05_SIXWAY")
