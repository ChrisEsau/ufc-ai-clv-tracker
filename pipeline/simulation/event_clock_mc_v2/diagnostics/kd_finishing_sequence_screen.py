"""Research-only men's KD-triggered finishing-sequence screen.

Keeps the validated consequence-side power curve fixed:
    power_offset(t) = clip(35 - t/12, -40, 35)

After a survived knockdown, this diagnostic resolves one abstract short
follow-up exchange immediately.  No additional strike attempts are inserted and
no existing event timestamps are changed.  The exchange stoppage probability is
anchored to the frozen shadow model's own post-KD KO probability, so fighter
power, defender age, elapsed-time consequence decay, and prior-KD state remain
inside the calculation.

Arms:
  control : no sequence resolver
  seq_b2  : logit(p_post_kd_ko) + 2.0
  seq_b3  : logit(p_post_kd_ko) + 3.0
  seq_b4  : logit(p_post_kd_ko) + 4.0

This is a falsification screen for the missing state-transition hypothesis, not
a production proposal or calibration.
"""
...
class ContinuousPowerShadow(FrozenShadow):
    ...
    def resolve_landed_strike(self, *, state, attacker, prior_defender_kds, rng):
        return self._shifted_model(state).resolve_landed_strike(
            state=state,
            attacker=attacker,
    ...
    def post_kd_ko_probability(self, *, state, attacker: Side, new_prior_kds: int) -> float:
        return float(self._shifted_model(state).ko_probability(
            state=state,
            attacker=attacker,
            prior_defender_kds=int(new_prior_kds),
        ))


def sequence_simulate_detailed_path(fight, budgets, submission_rates, conversion_probability, judge_model, judge_features, seed):
    global _MODE
        ...
        if landed and family in ("standing_strike", "ground_strike"):
            ...
            if consequence.knockdown:
                ...
                if bonus is not None:
                    p_post = shadow_ko_kd.post_kd_ko_probability(
                        state=state,
                        attacker=side,
                        new_prior_kds=kds[side_name],
                    )
                    p_sequence = _sigmoid(_logit(p_post) + float(bonus))
                    stats[side_name]["sequence_opportunities"] += 1
                    stats[side_name]["sequence_probability_sum"] += p_sequence
                    if rng.random() < p_sequence:
                        finish_time = min(float(state.fight_time_seconds) + SEQUENCE_SECONDS, horizon)
                        advance_to(finish_time)
                        stats[side_name]["sequence_finishes"] += 1
                        state.finished = True
                        state.finish_reason = "KO_TKO"
                        state.finish_method = "KO_TKO"
                        state.winner = side_name
    ...
    out = {
        ...
        "sequence_opportunities": stats["red"]["sequence_opportunities"] + stats["blue"]["sequence_opportunities"],
        "sequence_finishes": stats["red"]["sequence_finishes"] + stats["blue"]["sequence_finishes"],
    }
    for side_name in ("red", "blue"):
        sa = stats[side_name]["standing_strike_attempted"]
        sl = stats[side_name]["standing_strike_landed"]
        ga = stats[side_name]["ground_strike_attempted"]
        gl = stats[side_name]["ground_strike_landed"]
        out.update({
...
def _historical_targets(target_n):
    ...
    return (
        float(method.eq("KO_TKO").mean()),
        {t: float(((method != "DEC") & elapsed.le(t)).mean()) for t in (300, 600, 900)},
        float(elapsed.mean()),
    )
...
def _run_arm(target_n, sim_n, paths, seed, arm):
    global _MODE
    _MODE = arm
    runner.simulate_detailed_path = sequence_simulate_detailed_path
    canonical.simulate_detailed_path = sequence_simulate_detailed_path
    frames = []
...
def _summarize(summary, hist_ko, hist_finish, hist_elapsed, paths, arm):
    ...
    rec = {
        ...
        "ml_logloss": float(-np.mean(np.log(np.clip(winner_p, 1e-9, 1)))),
        ...
        "duration_relative_bias": float(summary["sim_mean_elapsed"].mean() / hist_elapsed - 1),
    ...
    for t in (300, 600, 900):
        ...
        rec[f"hist_nondec_by_{t}"] = hist_finish[t]
        rec[f"sim_nondec_by_{t}"] = sim
        rec[f"bias_nondec_by_{t}"] = sim - hist_finish[t]
    return rec
...
def main():
    ...
    ap.add_argument("--sim-n-per-division", type=int, default=20)
    ap.add_argument("--paths", type=int, default=10)
    ap.add_argument("--seed", type=int, default=20260823)
    ap.add_argument("--out-dir", type=Path, default=Path("data/diagnostics/event_clock_mc_v2/kd_finishing_sequence_screen"))
    args = ap.parse_args()
    ...
    summaries = []
    for arm in ARMS:
        s = _run_arm(args.target_n, args.sim_n_per_division, args.paths, args.seed, arm)
        s["arm"] = arm
        summaries.append(s)
        metrics.append(_summarize(s, hist_ko, hist_finish, hist_elapsed, args.paths, arm))

    m = pd.DataFrame(metrics)
    all_s = pd.concat(summaries, ignore_index=True)
    args.out_dir.mkdir(parents=True, exist_ok=True)
    m.to_csv(args.out_dir / "arm_metrics.csv", index=False)
    all_s.to_csv(args.out_dir / "fight_summaries.csv", index=False)

    print("\nHISTORICAL MEN TARGETS")
    print(f"KO share={hist_ko:.5f} | mean elapsed={hist_elapsed:.2f}s")
    for t in (300, 600, 900):
        print(f"nondecision by {t}s={hist_finish[t]:.5f}")
    print("\nARM METRICS")
