"""Run the requested 9-fight cohort through the age-adjusted total-KO V3 research shadow.

Research-only. Production unchanged.
KO architecture:
  - one total KO/TKO hazard per landed strike;
  - raw matchup hazard = 1-(1-attacker all KO wins/sig landed)*(1-defender all KO losses/sig absorbed);
  - chronological attacker/defender age slopes applied on log-odds;
  - if no KO, independently validated KD hazard may record a KD;
  - KD cannot finish and creates no post-KD finishing loop/hurt bridge.

Non-KO Brain, submissions, judging and strike-generation mechanics are frozen to the
same pure-EWM50 shadow setup used for Schnell-Costa.
"""
...
def logit(p):
    p = np.clip(float(p), 1e-9, 1.0 - 1e-9)
...
def resolve_target_rows(master: pd.DataFrame):
    out = []
    for a, b in TARGETS:
        m = master[((master.r_name.astype(str).eq(a)) & (master.b_name.astype(str).eq(b))) |
                   ((master.r_name.astype(str).eq(b)) & (master.b_name.astype(str).eq(a)))].copy()
        if len(m) != 1:
            raise RuntimeError(f"Expected one fight for {a} vs {b}, found {len(m)}")
        r = m.iloc[0]
        out.append({"fight_id": str(r.fight_id), "date": pd.Timestamp(r.date).normalize(), "a": a, "b": b,
                    "actual_method": str(r.method), "actual_winner_id": str(r.winner_id)})
    return out


def fit_age_slopes(frame: pd.DataFrame, cutoff: pd.Timestamp):
    ...
    ex=tr.loc[ids].copy()
    X=np.column_stack([ex.attacker_age.to_numpy(float)-30.0, ex.defender_age.to_numpy(float)-30.0])
    model=LogisticRegression(C=1.0,max_iter=5000,solver="lbfgs")
    model.fit(X,np.asarray(ys,int),sample_weight=np.asarray(ws,float))
    return float(model.coef_[0][0]), float(model.coef_[0][1]), {
        "fit_rows": int(len(tr)), "fit_ko_wins": int(tr.ko_win.sum()), "fit_sig_landed": float(tr.sig_landed.sum()),
        "attacker_age_logodds_per_year": float(model.coef_[0][0]),
        "defender_age_logodds_per_year": float(model.coef_[0][1]),
    }


def total_hazards_for_fight(frame: pd.DataFrame, fight_id: str, beta_att: float, beta_def: float):
    ...
    for row in target.itertuples(index=False):
        att_n=float(row.prior_sig_landed); def_n=float(row.opp_prior_sig_absorbed)
        att_k=float(row.prior_ko_wins); def_k=float(row.opp_prior_ko_losses)
        p_att=att_k/att_n if att_n>0 else 0.0; p_def=def_k/def_n if def_n>0 else 0.0
        p_raw=1.0-(1.0-p_att)*(1.0-p_def)
        delta=beta_att*(float(row.attacker_age)-30.0)+beta_def*(float(row.defender_age)-30.0)
        p_age=float(sigmoid(logit(p_raw)+delta)) if p_raw>0 else 0.0
        out[str(row.fighter_id)]={
            "fighter_name":str(row.fighter_name),"attacker_age":float(row.attacker_age),"defender_age":float(row.defender_age),
            "attacker_ko_per_sig":p_att,"defender_ko_loss_per_sig":p_def,
            "raw_total_ko_per_landed":p_raw,"age_logodds_delta":delta,"total_ko_per_landed":p_age,
        }
    return out


class Resolver:
    def __init__(self,total_by_side,kd_by_side):
        self.total_by_side=total_by_side; self.kd_by_side=kd_by_side; self.landed=Counter(); self.kos=Counter(); self.kds=Counter()
    def __call__(self,*,state,attacker_side,attacker,defender,rng):
        del attacker,defender
        target=state.physiology.fighter(attacker_side.opponent); prior=int(target.knockdowns_suffered)
        self.landed[attacker_side]+=1; p_ko=float(self.total_by_side[attacker_side]["total_ko_per_landed"])
        if rng.random()<p_ko:
            self.kos[attacker_side]+=1; return ko_mod.EmpiricalKOKDResult(p_ko,True,0.0,False,prior)
...
def main():
    ...
    targets=resolve_target_rows(master)
    ...
    canonical=pd.read_parquet(FSR_V3_PREFIGHT_SNAPSHOTS_PATH).copy(); canonical["event_date"]=pd.to_datetime(canonical.event_date).dt.normalize(); canonical["fight_id"]=canonical.fight_id.astype(str); canonical["fighter_id"]=canonical.fighter_id.astype(str)
    ewm50=build_pure_ewm50_snapshot(canonical)
    shutil.copy2(FSR_V3_PREFIGHT_SNAPSHOTS_PATH,BACKUP_PATH)
    original_standing=intent_mod._standing_rates; original_resolver=physiology_mod.resolve_empirical_ko_kd; original_hurt=physiology_mod.EMPIRICAL_KD_HURT_ACUTE_INCREMENT
    results=[]
    try:
        ...
        for item in targets:
            fight_id=item["fight_id"]; cutoff=item["date"]
            beta_att,beta_def,age_fit=fit_age_slopes(frame,cutoff)
            total_by_id=total_hazards_for_fight(frame,fight_id,beta_att,beta_def)
            kd_by_id=fit_prefight_hazards(fight_id=fight_id)
            pressure_mod.FIGHT_ID=fight_id; pressure_mod.PATHS=PATHS; intent_mod.FIGHT_ID=fight_id; intent_mod.PATHS=PATHS
            fight,inputs,priors,horizon,cfg=pressure_mod.build_setup(); side_to_id={Side.RED:str(fight.r_id),Side.BLUE:str(fight.b_id)}
            total_by_side={s:total_by_id[fid] for s,fid in side_to_id.items()}; kd_by_side={s:kd_by_id[fid] for s,fid in side_to_id.items()}
            resolver=Resolver(total_by_side,kd_by_side); physiology_mod.resolve_empirical_ko_kd=resolver
            brain=intent_mod.IntentRateBrain(inputs,priors,horizon); funcs=EngineFunctions(timing_sampler=brain.timing_sampler,action_chooser=brain.action_chooser)
            wins=Counter(); six=Counter()
            ...
            results.append({"fight_id":fight_id,"date":str(cutoff.date()),"fighters":rows,"age_fit":age_fit,"actual_method":item["actual_method"],"production_changed":False,"kd_can_finish":False,"post_kd_finish_loop":False})
            print(f"DONE {item['a']} vs {item['b']}")
        payload={"diagnostic":"KO V3 age-adjusted total-KO 9-fight cohort","paths_per_fight":PATHS,"standing_attempt_scale":STANDING_ATTEMPT_SCALE,"production_changed":False,"results":results}
