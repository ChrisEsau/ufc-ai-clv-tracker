...
def rate(n, seconds):
    return float(n * 900 / seconds / 2) if seconds else 0.0
...
def quantiles(xs):
    ...
    return {
        ...
        "p90": float(np.quantile(a, 0.9)) if a.size else 0.0,
    }
...
def main():
    ...
    p.add_argument("--paths-per-fight", type=int, default=20)
    p.add_argument("--exclude-discovery-fights", type=int, default=25)
    p.add_argument("--clinch-ratio", type=float, default=0.06)
    ...
    p.add_argument(
        ...
        type=Path,
        default=Path(
            "data/diagnostics/event_clock_mc_v2/stage8_holdout_validation.json"
        ),
    )
    a = p.parse_args()
    ...
    for _, fight in master.iterrows():
        ...
        try:
            ...
            rf, bf = historical_fighter_rows(
                snapshots,
                event_date=fight["_event_date"],
                fight_id=fid,
                fighter_ids=(rid, bid),
            )
            side_rows(rounds, fid, rid, "red")
            side_rows(rounds, fid, bid, "blue")
        except Exception:
            continue
        complete.append((fight, rf, bf))
        if len(complete) >= a.exclude_discovery_fights + a.fights:
            break
    if len(complete) < a.exclude_discovery_fights + a.fights:
        raise RuntimeError(f"insufficient complete fights: {len(complete)}")
    discovery = [str(x[0]["fight_id"]) for x in complete[: a.exclude_discovery_fights]]
    chosen = complete[a.exclude_discovery_fights :]
    counts = Counter()
    ...
    for fi, (fight, rf, bf) in enumerate(chosen):
        ...
        chooser = IntentPriorChooser(
            {
                Side.RED: BrainIntentPriors(
                    rr.standing_rate_15m,
                    rr.takedown_rate_15m,
                    a.clinch_ratio,
                    a.ground_strike_multiplier,
                    a.submission_multiplier,
                ),
                Side.BLUE: BrainIntentPriors(
                    br.standing_rate_15m,
                    br.takedown_rate_15m,
                    a.clinch_ratio,
                    a.ground_strike_multiplier,
                    a.submission_multiplier,
                ),
        ...
        for side, fid in [("red", rid), ("blue", bid)]:
            vals = actual_side_totals(
                side_rows(rounds, str(fight["fight_id"]), fid, side)
            )
            actual["standing"] += vals["distance_att"]
            actual["clinch"] += vals["clinch_att"]
            actual["ground"] += vals["ground_att"]
            actual["td"] += vals["td_att"]
            actual["td_landed"] += vals["td_land"]
            actual["sub"] += vals["sub_att"]
            actual_seconds += horizon
        caps = {Side.RED: (rc, rr), Side.BLUE: (bc, br)}
        for pi in range(a.paths_per_fight):
            out = run_causal_path(
                inp,
                seed=a.seed_base + fi * 10000 + pi,
            ...
            for s in out.timeline_segments:
                phases[s.phase.value] += s.duration
                durations[s.phase.value].append(s.duration)
                exposure += s.duration
            ...
            for e in out.events:
                ...
                if ac in STANDING:
                    counts["standing"] += 1
                elif ac is ActionFamily.CLINCH_STRIKE:
                    counts["clinch"] += 1
                elif ac in GROUND:
                    counts["ground"] += 1
                    counts[
                        (
                            "bottom_strike"
                            if ac is ActionFamily.BOTTOM_STRIKE
                            else "top_strike"
                        )
                    ] += 1
                if ac in TD:
                    ...
                    counts[
                        (
                            "direct_td"
                            if ac is ActionFamily.TAKEDOWN_ENTRY
                            else "clinch_td"
                        )
                    ] += 1
                    if e.resulting_phase is Phase.GROUND:
                        counts["td_landed"] += 1
                        counts["ground_entries"] += 1
                if ac is ActionFamily.SUBMISSION_ATTACK:
                    counts["sub"] += 1
                if ac is ActionFamily.CONTROL:
                    counts["control"] += 1
                if ac is ActionFamily.ESCAPE_STAND:
                    counts["escape_attempt"] += 1
                    counts["escape_success"] += e.resulting_phase is Phase.STANDING
                if ac is ActionFamily.REVERSAL:
                    counts["reversal_attempt"] += 1
                    counts["reversal_success"] += e.transition_kind is not None
                cap, rt = caps[e.actor]
                bucket = (
                    "low"
                    if cap.ground_top < 0.33
                    else "mid" if cap.ground_top < 0.67 else "high"
                )
                group[bucket][ac.value] += 1
    if illegal or mismatch:
        raise AssertionError({"illegal": illegal, "timeline_mismatch": mismatch})
    sim = {
        k: rate(counts[k], seconds)
        for k in (
            "standing",
            "clinch",
            "ground",
    ...
    act = {
        k: float(actual[k] * 900 / actual_seconds)
        for k in ("standing", "clinch", "ground", "td", "td_landed", "sub")
    }
    ground_minutes = phases["ground"] / 60
    payload = {
        "diagnostic": "Stage 8 fresh holdout",
        "discovery_fight_ids": discovery,
        "holdout_fight_ids": [str(x[0]["fight_id"]) for x in chosen],
        "fights": len(chosen),
        ...
        "phase_share": {k: v / sum(phases.values()) for k, v in phases.items()},
        "segment_seconds": {k: quantiles(v) for k, v in durations.items()},
        "ground_per_minute": {
            "top_control": counts["control"] / ground_minutes,
            "bottom_escape_attempts": counts["escape_attempt"] / ground_minutes,
            "ground_strikes": counts["top_strike"] / ground_minutes,
            "bottom_strikes": counts["bottom_strike"] / ground_minutes,
            "submissions": counts["sub"] / ground_minutes,
        },
        "success_rates": {
            "escape": counts["escape_success"] / max(1, counts["escape_attempt"]),
            "reversal": counts["reversal_success"] / max(1, counts["reversal_attempt"]),
        },
        "ground_action_distribution_by_capability": {
            k: dict(v) for k, v in group.items()
        },
    }
