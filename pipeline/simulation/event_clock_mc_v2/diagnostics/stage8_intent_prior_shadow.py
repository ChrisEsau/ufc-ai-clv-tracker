"""Stage 8 shadow cohort for the FSR intent-prior policy candidate.

Production policy is unchanged.  This diagnostic injects an intent-aware chooser
through the Stage-6 EngineFunctions seam so the causal engine, timing and
mechanics remain identical to the approved refactor.
"""
...
from pipeline.simulation.event_clock_mc_v2.brain.intent_priors import (
    BrainIntentPriors,
    choose_action_with_intent_priors,
)
...
class IntentPriorChooser:
    ...
    def __call__(self, state, actor, capabilities, context, rng, config) -> ActionFamily:
        return choose_action_with_intent_priors(
            state,
            actor,
            capabilities,
            context,
...
def main() -> None:
    ...
    ap.add_argument(
        "--output",
        type=Path,
        default=Path("data/diagnostics/event_clock_mc_v2/stage8_intent_prior_shadow.json"),
    )
    args = ap.parse_args()
        ...
        chooser = IntentPriorChooser(
            {
                Side.RED: BrainIntentPriors(
                    red_runtime.standing_rate_15m, red_runtime.takedown_rate_15m
                ),
                Side.BLUE: BrainIntentPriors(
                    blue_runtime.standing_rate_15m, blue_runtime.takedown_rate_15m
                ),
            }
        )
        ...
        fight_rows.append(
            {
                ...
                "horizon_seconds": horizon,
                "red_standing_rate_per15": float(red_runtime.standing_rate_15m),
                "red_td_rate_per15": float(red_runtime.takedown_rate_15m),
                "blue_standing_rate_per15": float(blue_runtime.standing_rate_15m),
                "blue_td_rate_per15": float(blue_runtime.takedown_rate_15m),
            }
        )
    ...
    keys = ("standing_att", "clinch_att", "ground_att", "td_att", "td_land", "sub_att")
    payload = {
        "diagnostic": "Stage 8 FSR intent-prior shadow",
        "production_policy_changed": False,
        "semantics": (
            "neutral TD-vs-standing-strike odds anchored to matchup-effective FSR "
            "attempt-rate ratio; completion remains mechanics"
        ),
        "fights": len(selected),
        "paths_per_fight": args.paths_per_fight,
        "total_paths": total_paths,
        "invariants": {
            "illegal_ground_standing_actions": illegal,
            "timeline_exposure_mismatches": exposure_mismatch,
        },
        "simulated_phase_share": {
            key: value / phase_total for key, value in phase_seconds.items()
        },
        "actual_vs_shadow_per15": {
            key: {
                "actual": per15(actual[key], actual_seconds),
                "shadow": per15(sim[key], actual_seconds),
            }
            for key in keys
        },
    ...
    print("=" * 100)
    print("STAGE 8 FSR INTENT-PRIOR SHADOW")
    print("=" * 100)
