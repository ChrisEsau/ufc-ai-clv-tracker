...
def load_frozen_context(bundle_path: Path, target_ids: set[str]):
    if not bundle_path.exists():
        raise RuntimeError(
            f"Frozen Event Clock bundle not found: {bundle_path}\n"
            "Build it once with:\n"
            "  PYTHONPATH=. python -m "
            "pipeline.simulation.event_clock_mc_v1.fit_event_clock_bundle"
        )

    payload = joblib.load(bundle_path)
    if payload.get("schema_version") != 1:
        raise RuntimeError(
            f"Unsupported Event Clock bundle schema: "
            f"{payload.get('schema_version')!r}"
        )

    context = payload["context"]
    available = set(context["test"]["fight_id"].astype(str))
    missing = sorted(target_ids - available)
    if missing:
        raise RuntimeError(
            "Target fight(s) are outside this frozen bundle's prediction "
            "universe: " + ", ".join(missing) + "\n"
            "Refit a bundle whose prediction universe includes those fights."
        )

    # Never mutate the serialized full-cohort frame in memory beyond this run.
    loaded = dict(context)
    loaded["test"] = context["test"][
        context["test"]["fight_id"].astype(str).isin(target_ids)
    ].copy()
    return loaded
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Run Event Clock MC from a persisted frozen statistical bundle; "
            "no training models are refit during prediction."
        )
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--event", help="Case-insensitive substring of UFC master event_name.")
    group.add_argument("--event-date", help="Event date, e.g. 2026-06-20.")
    ...
    master["event_date"] = pd.to_datetime(
        master["date"], errors="raise"
    ).dt.normalize()

    target = select_target(master, args)
    target_ids = set(target["fight_id"].astype(str))
    context = load_frozen_context(args.bundle, target_ids)
    test = context["test"]
    ...
    if metadata:
        ...
        print(
            "bundle prediction dates: "
            f"{metadata.get('prediction_first_event_date')} through "
            f"{metadata.get('prediction_last_event_date')}"
        )
    ...
    for fight_index, (fight_id, pair) in enumerate(groups):
        ...
        summary_rows.append(
            summarize_fight(fight_id, pair, fight_rows, master_row)
        )
    ...
    print(
        f"Winner+method accuracy:   "
        f"{summary['winner_method_correct'].mean():.2%}"
    )
