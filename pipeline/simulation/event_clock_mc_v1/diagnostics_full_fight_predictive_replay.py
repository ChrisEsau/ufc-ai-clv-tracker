...
def simulate_integrated_path(
    ...
    physiology = ImpactTraumaKnockdownModel(
        physics_profiles,
        calibration,
    )

    ko_finish = KOTKOFinishModel(
        physics_profiles,
        calibration,
    )
    ...
    for (
        ...
        payload = ActionAttempt(
            ...
            dynamic_modifiers=dynamic,
            landed=bool(landed),
        )

        phys_delta, phys_events = (
            physiology.resolve(
                state,
                payload,
                state.fight_time_seconds,
