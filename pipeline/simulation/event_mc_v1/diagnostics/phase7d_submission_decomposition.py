...
def run(paths=10, start_year=2020, limit=100, seed=20260813,
    ...
    started = time.perf_counter(); rows = []; round_attempts = Counter(); position_attempts = Counter()
    ...
    summary = {
        ...
        "guardrails": {"kd_per_100_landed": float(frame.kds.sum() / frame.landed.sum() * 100), "kd_per_15min": float(frame.kds.sum() / exposure * 900), "ko_tko_share": float(methods.get("KO_TKO", 0))},
