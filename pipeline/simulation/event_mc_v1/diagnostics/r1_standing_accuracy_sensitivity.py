"""R1 standing-strike accuracy sensitivity for canonical FSR V2.

Fixed opportunity calibration:
    standing strike rate multiplier = 0.80
    ground strike rate multiplier   = 2.25

Only the standing accuracy population intercept is shifted.
"""
...
OFFSET_GRID = (0.15, 0.20, 0.22, 0.25, 0.30)
...
def logit_shift(p, offset):
    eps = 1e-9
    p = min(max(float(p), eps), 1.0 - eps)
    z = math.log(p / (1.0 - p)) + float(offset)
    return 1.0 / (1.0 + math.exp(-z))


def shifted_fight(fight, standing_offset):
...
def simulated_metrics(frame):
    ...
    return {
        ...
        "attempts_5": safe_div(total_a, exposure) * 300.0,
        "landed_5": safe_div(total_l, exposure) * 300.0,
        "r1_complete": float(frame["completed_r1"].mean()),
    }
...
def evaluate(name, cohort, fsr, rounds, paths, seed):
    ...
    for offset in OFFSET_GRID:
        ...
        rows.append({
            ...
            "hist_stand_acc": hist["standing_accuracy"],
            "sim_stand_acc": sim["standing_accuracy"],
            "stand_acc_err_%": pct(
                sim["standing_accuracy"],
                hist["standing_accuracy"],
            ),

            "hist_ground_acc": hist["ground_accuracy"],
            "sim_ground_acc": sim["ground_accuracy"],

            "hist_total_acc": hist["total_accuracy"],
            "sim_total_acc": sim["total_accuracy"],
            "total_acc_err_%": pct(
                sim["total_accuracy"],
                hist["total_accuracy"],
            ),
