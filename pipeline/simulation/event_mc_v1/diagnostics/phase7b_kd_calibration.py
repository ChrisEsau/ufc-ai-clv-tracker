...
def evaluate(cohort, fsr, midpoint, paths, seed=20260813):
    ...
    return {"midpoint":float(midpoint),"fights":len(cohort),"paths_per_fight":paths,"attempts_per_15min":frame.attempts.sum()/exposure*900,"landed_per_15min":landed/exposure*900,"landing_rate":landed/frame.attempts.sum(),"kd_per_100_landed":kd100,"kd_per_15min":kd15,"zero_kd_share":float((frame.kds==0).mean()),"multi_kd_share":float((frame.kds>=2).mean()),"kd_by_round":dict(kd_round),"kd_by_phase":dict(kd_phase),"method_shares":{m:float(methods.get(m,0)) for m in ("KO_TKO","SUB","DEC")},"mean_nondecision_finish_time":float(frame.loc[frame.method!="DEC","seconds"].mean()),"winner_accuracy":float(np.mean((p>=.5)==y)),"brier":float(np.mean((p-y)**2)),"log_loss":float(-np.mean(y*np.log(safe)+(1-y)*np.log(1-safe))),"normalized_error":abs(kd100/TARGET_KD_PER_100_LANDED-1)+abs(kd15/TARGET_KD_PER_15MIN-1),"runtime_seconds":time.perf_counter()-started}
