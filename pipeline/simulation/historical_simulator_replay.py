"""Historical holdout replay for the round-level fight simulator.

This module evaluates the current heuristic mechanics engine against completed
fights. Fighter states are reconstructed only from fights completed before the
target matchup. Actual target-fight outcomes and statistics are retained solely
for scoring after simulation.

The replay is intentionally diagnostic. It does not promote simulator outputs to
production or claim wagering-grade calibration.
"""
...
class HistoricalSimulatorReplayError(RuntimeError):
    """Raised when historical replay data or outputs violate the contract."""
...
def _scaled_ratio(value: float, reference: float, sensitivity: float = 0.75) -> float:
    ...
    return _clip_probability(0.5 + 0.34 * np.tanh(log(ratio) * sensitivity), 0.10, 0.90)


def _normalize_corner(value: object) -> str:
    text = str(value).strip().lower()
    if text not in {"red", "blue"}:
        raise HistoricalSimulatorReplayError(f"Invalid corner value: {value!r}")
    return text
...
def _coerce_training_frame(training_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["corner"] = df["corner"].map(_normalize_corner)
    numeric = (
        "round",
        "total_rounds",
        "match_time_sec",
    ...
    if df[list(numeric)].isna().any().any():
        raise HistoricalSimulatorReplayError("Replay rows contain missing numeric values")
...
def build_fighter_fight_history(training_df: pd.DataFrame) -> pd.DataFrame:
    ...
    paired_columns = [
        "fight_id",
        "fighter_id",
        *[f"fight_{name}" for name in STAT_TARGETS],
    ]
    opponent = fights[paired_columns].rename(
        columns={
            "fighter_id": "opponent_id",
            **{
                column: column.replace("fight_", "fight_allowed_", 1)
                for column in paired_columns
                if column.startswith("fight_")
            },
        }
    )
    fights = fights.merge(
    ...
    if fights[allowed_columns].isna().any().any():
        raise HistoricalSimulatorReplayError(
    ...
    cumulative_sources = [*PRIOR_BASE_COLUMNS, *allowed_columns]
    for column in cumulative_sources:
        values = pd.to_numeric(fights[column], errors="coerce").fillna(0.0)
        fights[column] = values
        fights[f"prior_{column}"] = group[column].cumsum() - values
...
def population_priors(history_df: pd.DataFrame, test_year: int) -> dict[str, float]:
    """Estimate fixed cold-start priors using only fights before the holdout year."""
    ...
    return {
        ...
        "sig_allowed_accuracy": _safe_divide(
            allowed_sig_landed, allowed_sig_attempted, 0.45
        ),
        ...
        "td_allowed_accuracy": _safe_divide(
            allowed_td_landed, allowed_td_attempted, 0.35
        ),
        "control_seconds_per_td": _safe_divide(
            float(history["fight_control_seconds"].sum()), td_landed, 45.0
        ),
        "kd_per_sig_landed": _safe_divide(
            float(history["fight_knockdowns"].sum()), sig_landed, 0.015
        ),
        ...
        "ko_win_rate": _safe_divide(float(history["fight_ko_win"].sum()), fights, 0.12),
        "ko_loss_rate": _safe_divide(float(history["fight_ko_loss"].sum()), fights, 0.12),
        "sub_win_rate": _safe_divide(float(history["fight_sub_win"].sum()), fights, 0.08),
        "sub_loss_rate": _safe_divide(float(history["fight_sub_loss"].sum()), fights, 0.08),
...
def fighter_state_from_history(
    ...
    """Translate one shifted fighter-fight history row into simulator state."""
    ...
    kd_rate = _safe_divide(
        float(row["prior_fight_knockdowns"]) + priors["kd_per_sig_landed"] * 75.0,
    ...
    initiative = _clip_probability(
        _safe_divide(own_attempts + 50.0, own_attempts + allowed_attempts + 100.0, 0.5),
        0.18,
        0.82,
    )
    ...
    phase_imposition = _clip_probability(0.55 * control_share + 0.45 * td_share, 0.15, 0.85)
...
def _actual_method(value: object) -> str | None:
...
def build_holdout_matchups(
    ...
    """Build paired matchups and actual scoring records for one holdout year."""
    candidates = history_df.loc[history_df["date"].dt.year.eq(int(test_year))].copy()
    candidates = candidates.loc[candidates["method_family"].isin(METHODS)].copy()
    if candidates.empty:
        raise HistoricalSimulatorReplayError(
    ...
    for fight_id, group in candidates.groupby("fight_id", sort=False):
        ...
        if winner_id not in {str(red["fighter_id"]), str(blue["fighter_id"])}:
            continue
        method = _actual_method(red["method_family"])
        if method is None:
            continue
    ...
    records.sort(key=lambda row: (row["date"], row["matchup"].fight_id))
...
def _fight_level_baselines(
    ...
    for fight_id, group in pretest.groupby("fight_id", sort=False):
        if len(group) != 2 or set(group["corner"]) != {"red", "blue"}:
            continue
        red = group.loc[group["corner"].eq("red")].iloc[0]
        blue = group.loc[group["corner"].eq("blue")].iloc[0]
        method = _actual_method(red["method_family"])
        winner_id = str(red["winner_id"])
        if method is None or winner_id not in {
            str(red["fighter_id"]),
            str(blue["fighter_id"]),
        }:
            continue
    ...
    if frame.empty:
        raise HistoricalSimulatorReplayError("No pretest fights were available for baselines")
...
def run_historical_simulator_replay(
    ...
    """Simulate completed holdout fights and compare with actual outcomes."""
    ...
    for index, record in enumerate(matchups):
        ...
        summary, _ = run_simulation(matchup, runtime)
        ...
        total_method_probability = sum(method_probabilities.values())
        if total_method_probability <= 0:
            raise HistoricalSimulatorReplayError(
                f"Simulator returned zero method mass for {matchup.fight_id}"
            )
        method_probabilities = {
            key: value / total_method_probability
        ...
        prediction_rows.append(
            {
                ...
                "simulations": simulations_per_fight,
    ...
    aggregate = aggregate_comparison(predictions)
...
def _multiclass_metrics(
    ...
    mapping = {method: index for index, method in enumerate(METHODS)}
    actual_index = np.asarray([mapping[str(value)] for value in actual_method], dtype=int)
    ...
    picked = np.argmax(probability, axis=1)
    return {
        "accuracy": float(np.mean(picked == actual_index)),
        "log_loss": float(-np.mean(np.log(probability[np.arange(len(actual_index)), actual_index]))),
    }
...
def score_historical_replay(predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    for model, prefix in (
        ...
        for metric, value in _multiclass_metrics(
            predictions["actual_method"], probability
        ).items():
            rows.append(
                {
                    "task": "method",
        ...
        distance_probability = predictions[f"{prefix}_decision_probability"]
        for metric, value in _binary_metrics(
            actual_distance, distance_probability
        ).items():
            rows.append(
                {
                    "task": "goes_distance",
    ...
    for model, column in (
        ...
        for metric, value in _continuous_metrics(
            predictions["actual_fight_time_seconds"], predictions[column]
        ).items():
            rows.append(
                {
    ...
    for model, red_column, blue_column in (
        ...
        for metric, value in _continuous_metrics(
            actual_strikes, predicted_strikes
        ).items():
            rows.append(
                {
...
def calibration_tables(predictions: pd.DataFrame, bins: int = 5) -> pd.DataFrame:
    ...
    for task, actual_column, probability_column in (
        ...
        if task == "winner_red":
            frame["actual"] = frame[actual_column].eq("red").astype(float)
        else:
            frame["actual"] = frame[actual_column].eq("decision").astype(float)
    ...
    rows = [
        {
            ...
            "actual": float(predictions["actual_winner_corner"].eq("red").mean()),
        ...
        },
        {
            ...
            "simulator": float(predictions["sim_submission_probability"].mean()),
        },
        {
...
def metric_lookup(
    ...
    if len(match) != 1:
        raise HistoricalSimulatorReplayError(
            f"Metric lookup did not resolve uniquely: {task}/{model}/{metric}"
        )
