...
## Judging and terminal submissions

Repository and data inventory found UFCStats round action totals and a
`stage10b_old_judge_rounds.csv` diagnostic whose round winners were generated by
an old heuristic. It did **not** find official per-judge 10-9/10-8 scorecards or
trustworthy historical round-winner labels. The V2 judge is consequently named
`EVENT2_TOTAL_JUDGE_ROUND_TRANSFER`: the exact Stage-10D `FULL_TOTAL` equation
(`sig_diff`, `kd_diff`, `td_diff`, `sub_diff`, `ctrl_diff`) is applied separately
to each completed round. This is a transfer assumption and is not yet
round-calibrated.

Three isolated judge draws are made for every completed round. Each winner gets
10-9, each judge totals only their own card, and two of three cards select the
official winner. With no draw rounds or point deductions, classifications are
only unanimous or split decision. Scorecards are terminal outputs and are not
fed into Brain `score_state`.

FSR V3 publishes exact prefight `submission_conversion_baseline`,
`submission_offense`, and `submission_defense` equivalents. Stage-11C researched
`logistic(logit(baseline) + offense - defense)`, but the later accepted integrated
Event2 replay superseded it with the fitted training-cohort global conversion
offset: `logistic(logit(baseline) + conversion_offset)`. The runner reads that
unchanged offset from the approved frozen V2 bundle. Offense and defense are
retained for provenance/allocation diagnostics but are not silently reintroduced
into the final integrated conversion equation.

The Brain remains the only source of legal `SUBMISSION_ATTACK` attempts. The
submission module resolves conversion only, on a seventh isolated engine RNG
stream. Success terminates immediately; failure remains an authoritative attempt
for round judging. An eighth isolated stream owns judging. `spawn(8)` preserves
the identities of the existing first six streams.
