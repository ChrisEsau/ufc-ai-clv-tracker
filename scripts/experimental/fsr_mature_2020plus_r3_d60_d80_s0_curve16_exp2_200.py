"""Research-only comparison of aggressive R3 damage recovery with zero stamina recovery.

R2 entry after R1 is held fixed at the current policy:
- damage recovery: 20% of missing
- stamina recovery: 40% of missing

Two candidates are tested after R2, entering R3:
A) damage 60%, stamina 0%
B) damage 80%, stamina 0%
...
OUTPUT_PATH = Path("data/experimental/r3_d60_d80_s0_curve16_exp2_200.csv")
...
CANDIDATES = (
    RecoveryCandidate("r3_d60_s0", 0.60, 0.00),
    RecoveryCandidate("r3_d80_s0", 0.80, 0.00),
)
    ...
    for bout_idx, (_, bout) in enumerate(cohort.iterrows()):
        ...
        for seed in seed_matrix[bout_idx]:
            sim1, path1, kd1, fr1 = _run_prefix(
                red, blue, candidate=candidate, rounds=1, seed=int(seed), red_age=r_age, blue_age=b_age
            )
            sim2, path2, kd2, fr2 = _run_prefix(
                red, blue, candidate=candidate, rounds=2, seed=int(seed), red_age=r_age, blue_age=b_age
            )
            sim3, path3, kd3, fr3 = _run_prefix(
                red, blue, candidate=candidate, rounds=3, seed=int(seed), red_age=r_age, blue_age=b_age
            )
...
def main() -> None:
    ...
    print("\n" + "=" * 160)
    print("R3 AGGRESSIVE DAMAGE RECOVERY COMPARISON — CURVE 16 / EXPONENT 2.0 — EXACT SAME 200 BOUTS x 10 PATHS")
    print("=" * 160)
    print("R2 entry fixed: damage=20% of missing; stamina=40% of missing")
    print("Run A R3 entry: damage=60%; stamina=0%")
    print("Run B R3 entry: damage=80%; stamina=0%")
