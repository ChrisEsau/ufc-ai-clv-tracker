...
Decision cohort
---------------
RFS history carries completed-fight round count and KO/submission loss indicators,
but not the raw outcome method string. Therefore this script defines the V0
full-distance cohort as fights where both fighter rows:
- have the same observed round count of exactly 3 or 5;
- have no KO/TKO loss indicator;
- have no submission loss indicator.

This is an extremely close proxy for standard decisions; unusual non-KO/non-sub
terminal methods that survive the full scheduled round count are possible but rare.
The script reports cohort size explicitly.
"""
...
def _attach_realized(sim: pd.DataFrame, decision_rows: pd.DataFrame) -> pd.DataFrame:
    ...
    for metric in (
        ...
        realized[f"real_{metric}"] = pd.to_numeric(
            decision_rows[RFS[metric]], errors="coerce"
        ).to_numpy()

    realized["real_sig_accuracy"] = np.divide(
        realized["real_sig_landed"],
        realized["real_sig_att"],
        out=np.full(len(realized), np.nan),
        where=realized["real_sig_att"].to_numpy(dtype=float) > 0,
    )
    realized["real_td_completion"] = np.divide(
        realized["real_td_landed"],
        realized["real_td_att"],
        out=np.full(len(realized), np.nan),
...
def main() -> None:
    ...
    # Require two leakage-safe FSR snapshots for each historical matchup.
    ...
    if (audit["scheduled_rounds"] == 3).any():
        _metric_table(audit[audit["scheduled_rounds"] == 3], "THREE-ROUND FULL-DISTANCE FIGHTS")
    if (audit["scheduled_rounds"] == 5).any():
        _metric_table(audit[audit["scheduled_rounds"] == 5], "FIVE-ROUND FULL-DISTANCE FIGHTS")

    args.output.parent.mkdir(parents=True, exist_ok=True)
    audit.to_parquet(args.output, index=False)
    print(f"\n[decision audit] wrote {args.output}", flush=True)
    print(
        "\nINTERPRETATION: means/bias test absolute calibration; MAE/RMSE test fighter-fight "
        "error; Spearman tests whether leakage-safe FSR matchup differences rank realized "
        "fight statistics. V0 finishes and dynamic state remain disabled."
    )
