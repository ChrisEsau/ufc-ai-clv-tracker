"""Research-only KD-collapse curvature sweep with scale fixed at 2.0.

Purpose
-------
Start from a slightly KD-rich collapse scale, then reintroduce nonlinearity to
selectively convert the highest-shock knockdowns into KO/TKO finishes.
...
- Collapse scale fixed at 2.0.
- Terminal collapse is KO/TKO only; only surviving knockdowns count as KD.

200 bouts x 10 paths. CSV is rewritten after every completed curvature.
"""
...
COLLAPSE_SCALE = 2.0
CURVATURES = (0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0, 6.0)

OUTPUT_PATH = Path("data/experimental/collapse_curvature_sweep_scale2_200.csv")
...
class CurvatureScale2AuditSim(v33.StaticFSRMCKOTKOV33GlobalRecovery):
...
def main() -> None:
    ...
    print("\n" + "=" * 150)
    print("KD COLLAPSE CURVATURE SWEEP — SCALE FIXED AT 2.0 — 200 BOUTS x 10 PATHS")
    print("=" * 150)
    ...
    out["kd_fit"] = (
        ((out["r1_kd_mean"] - HIST_R1_KD_MEAN) / HIST_R1_KD_MEAN) ** 2
        + ((out["total_kd_mean"] - HIST_TOTAL_KD_MEAN) / HIST_TOTAL_KD_MEAN) ** 2
        + ((out["any_kd"] - HIST_ANY_KD) / HIST_ANY_KD) ** 2
    )
    out["r1_ko_gap"] = (out["r1_ko"] - HIST_R1_KO).abs()
    out = out.sort_values(["r1_ko_gap", "kd_fit", "curvature"]).reset_index(drop=True)
    ...
    print("\nRANKED — PRIORITIZE R1 KO, THEN KD FIT")
    print(
        f"{'candidate':>22} {'R1KD':>8} {'R2KD':>8} {'R3KD':>8} {'KDtot':>8} {'KDany':>8} "
        f"{'R1KO':>8} {'R2KO':>8} {'R3KO':>8} {'KOtot':>8} {'meanRnd':>8} {'termKO':>8} {'direct':>8}"
    )
    for _, row in out.iterrows():
        print(
            f"{row['candidate']:>22} {row['r1_kd_mean']:8.4f} {row['r2_kd_mean']:8.4f} {row['r3_kd_mean']:8.4f} "
            f"{row['total_kd_mean']:8.4f} {row['any_kd']:8.2%} {row['r1_ko']:8.2%} {row['r2_ko']:8.2%} "
            f"{row['r3_ko']:8.2%} {row['total_ko']:8.2%} {row['mean_ko_round']:8.3f} "
            f"{row['terminal_collapse_ko']:8.2%} {row['direct_strike_ko']:8.2%}"
        )
