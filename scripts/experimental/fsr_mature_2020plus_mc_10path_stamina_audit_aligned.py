"""Aligned population audit for the FSR-32 stamina KO/TKO candidate.

Uses the same mature 2020+ cohort, locked age mechanic, strong KD collapse, and
seed schedule as the current recovery audit, but consumes FSR-32 profiles and
runs the stamina-aware V3 engine.
"""
...
OUTPUT_PATH = Path(
    ...
    "mature_2020plus_mc_10path_stamina_audit_aligned.csv"
)
...
def _simulate_one_bout(
    ...
    *,
    ...
    r_output_end = b_output_end = 0.0
    r_power_end = b_power_end = 0.0

    for seed in seeds:
        sim = stamina.StaticFSRMCKOTKOV3Stamina(
            red,
            blue,
        ...
        r_output_end += sim.stamina_output_multiplier(0)
        b_output_end += sim.stamina_output_multiplier(1)
        r_power_end += sim.stamina_power_multiplier(0)
        b_power_end += sim.stamina_power_multiplier(1)
    ...
    return {
        ...
        "mean_r_final_output_multiplier": r_output_end / paths,
        "mean_b_final_output_multiplier": b_output_end / paths,
        "mean_r_final_power_multiplier": r_power_end / paths,
        "mean_b_final_power_multiplier": b_power_end / paths,
    }


def _print_summary(frame: pd.DataFrame, paths: int, rounds: int) -> None:
    print("\n" + "=" * 132)
    print("MATURE 2020+ ALIGNED MC AUDIT — FSR-32 STAMINA + STRONG KD COLLAPSE + LOCKED AGE")
    print("=" * 132)
    ...
    print(
        "mean final output multiplier: "
        f"red={frame['mean_r_final_output_multiplier'].mean():.3f}, "
        f"blue={frame['mean_b_final_output_multiplier'].mean():.3f}"
    )
    print(
        "mean final power multiplier: "
        f"red={frame['mean_r_final_power_multiplier'].mean():.3f}, "
        f"blue={frame['mean_b_final_power_multiplier'].mean():.3f}"
    )
...
def main() -> None:
    ...
    for bout_no, (_, bout) in enumerate(cohort.iterrows(), start=1):
        ...
        if completed % 1000 == 0 or bout_no == len(cohort):
            print(f"[stamina MC] paths {completed:,}/{total:,}; bouts {bout_no:,}/{len(cohort):,}", flush=True)
    ...
    print("FSR-28 and the current recovery simulator remain unchanged.")
