"""Paired single-fight comparison: current recovery baseline vs rolling FSR V3.1."""
...
def _run(sim_cls, red, blue, *, seeds, rounds, r_age, b_age):
    ...
    for seed in seeds:
        ...
        for i, stats in enumerate(sim.stats):
            ...
            if isinstance(sim, rolling.StaticFSRMCKOTKOV31RollingFSR):
                totals[i]["final_stamina"] += sim.stamina_state[i].fraction
                totals[i]["final_fatigue_penalty"] += sim.fatigue_penalty(i)
                totals[i]["final_effective_power"] += float(
                    sim._effective_profile(i)["striking_power"]
                )
            else:
...
def main() -> None:
    ...
    seeds = np.random.default_rng(args.seed).integers(
        0, 2**31 - 1, size=args.paths, dtype=np.int64
    )

    baseline = _run(
        recovery.StaticFSRMCKOTKOV2RoundRecovery, red, blue, seeds=seeds,
        rounds=args.rounds, r_age=r_age, b_age=b_age,
    )
    candidate = _run(
        rolling.StaticFSRMCKOTKOV31RollingFSR, red, blue, seeds=seeds,
        rounds=args.rounds, r_age=r_age, b_age=b_age,
    )

    names = [base._display_name(red), base._display_name(blue)]
    print("\n" + "=" * 126)
    print("ALIGNED SINGLE-FIGHT ROLLING-FSR STAMINA COMPARISON")
    print("=" * 126)
    ...
    fields = [
        "striking_power", fsr32.STAMINA_CAPACITY,
        fsr32.STAMINA_DEPLETION_RESISTANCE,
        fsr32.STAMINA_PERFORMANCE_RESILIENCE,
        fsr32.STAMINA_RECOVERY_ABILITY,
    ]
    print(pd.DataFrame({names[0]: [red[f] for f in fields], names[1]: [blue[f] for f in fields]}, index=fields).to_string(float_format=lambda x: f"{float(x):.4f}"))

    rows = []
    for label, pair in (("baseline_recovery", baseline), ("rolling_fsr_v3_1", candidate)):
    ...
    print("\nROLLING-FSR DELTAS VS BASELINE")
