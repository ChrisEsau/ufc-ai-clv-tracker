"""Aligned mature 2020+ population audit for global-recovery V3.3.

Uses the same mature 2020+ aligned cohort, strong KD-collapse candidate,
three-round horizon, path count, and seed schedule as the V3.2 population audit.
The only candidate change is V3.3 recovery physics: global 20% of missing damage
reservoir and global 40% of missing stamina restored between survived rounds.
"""
...
OUTPUT_PATH = Path(
    ...
    "mature_2020plus_mc_10path_global_recovery_v33_audit_aligned.csv"
)
...
def _print_summary(frame: pd.DataFrame, paths: int, rounds: int) -> None:
    print("\n" + "=" * 132)
    print("MATURE 2020+ ALIGNED MC AUDIT — GLOBAL-RECOVERY V3.3 + STRONG KD COLLAPSE + LOCKED AGE")
    print("=" * 132)
...
def main() -> None:
    ...
    for bout_no, (_, bout) in enumerate(cohort.iterrows(), start=1):
        ...
        if completed % 1000 == 0 or bout_no == len(cohort):
            print(
                f"[global-recovery V3.3 MC] paths {completed:,}/{total:,}; "
    ...
    print("FSR-28, V3.1, V3.2, and the recovery baseline remain unchanged.")
